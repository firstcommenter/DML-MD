...
Please check your configuration or try again later.`
  ...
  if (autobio) {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        `${botname} is Active 24/7 WATU NI MTAJI TOSHA\n\n${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })} It is ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi' })}.`
      );
    }, 10 * 1000);
  }
  ...
  client.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    require("./daudi")(client, m, { type }, store);
  });
