...
module.exports = async (context) => {
    await ownerMiddleware(context, async () => {
        ...
        if (value === 'on' || value === 'off') {
            ...
            if (isEnabled === action) {
                return await m.reply(`✅ GC Presence is already ${value.toUpperCase()}.`);
            }

            await updateGroupSetting(jid, 'gcpresence', action ? 'true' : 'false');
            await m.reply(`✅ GC Presence has been turned ${value.toUpperCase()} for this group. Bot will now simulate fake typing and recording.`);
        } else {
