...
module.exports = async (context) => {
    ...
    try {
        ...
        const imageMsg =
            ...
            viewOnce?.imageMessageV2 ||
            viewOnce?.imageMessageV1;

        const videoMsg =
            viewOnce?.videoMessage ||
            viewOnce?.videoMessageV2 ||
            viewOnce?.videoMessageV1;

        if (!imageMsg && !videoMsg)
            return m.reply('This message does not contain view-once media.');
        ...
        // 🔑 FORCE SEND TO BOT INBOX (PRIVATE CHAT)
        const chatId = client.user.id;

        const caption = `🔰\n—\nDML-MD`;

        if (imageMsg) {
            await client.sendMessage(chatId, { image: buffer, caption });
        } else {
            await client.sendMessage(chatId, { video: buffer, caption });
        }
